...
# ------------------------------------------
    ...
    if st.button("📖 Kapsamlı Yeni Ders Hazırla", type="primary", use_container_width=True):
        with st.spinner("Alman profesör kapsamlı müfredatı ve kelimeleri hazırlıyor... (Bu işlem 10-15 saniye sürebilir)"):
            sys_prompt = "Sen Almanya'nın en efsanevi, en espritüel ve samimi profesörüsün. Öğrenciyle doğrudan konuşur, ona 'sen' diye hitap edersin. Asla kendi yapacağın şeyi raporlama, DOĞRUDAN KONUYU ANLAT."
            user_prompt = f"""
    ...
    if st.session_state.ders_icerigi:
        ...
        if sayfalar and 0 <= mevcut_s < len(sayfalar):
            ...
            # TR/DE etiketleri geliştirildi, okuma kolaylaştırıldı
            ornekler_html = ""
            for orn in sayfa_data.get('ornekler', []):
                ornekler_html += f"""
                <div style='margin-bottom:12px; padding: 15px; background: rgba(0,0,0,0.4); border-left: 4px solid #60a5fa; border-radius: 6px;'>
                    <div style='font-size:18px; color:#f8fafc; margin-bottom:4px;'>🇩🇪 {orn['de']}</div>
                    <div style='font-size:15px; color:#94a3b8;'>🇹🇷 {orn['tr']}</div>
                </div>
                """
            ...
            <div class="lesson-box">
                ...
                <p style="font-size: 18px; line-height: 1.8; color: #f8fafc; margin-bottom: 30px;">
...
# ------------------------------------------
    ...
    if "sp_history" not in st.session_state: 
        ...
        st.session_state.sp_history.append({
            "role": "ai", 
            "de": f"Hallo! Wie geht es dir heute?", 
    ...
    for msg in st.session_state.sp_history:
        if msg["role"] == "user":
            ...
            if msg.get("correction"):
                c_html = f'<div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 13px; color: #fbbf24;">💡 <b>Düzeltme:</b> {msg["correction"]}</div>'
...
# ------------------------------------------
    ...
    if not kelimeler:
        ...
        # Dinamik indeksleme: ValueError hatası (too many values to unpack) kesin çözüldü
        ...
        with col2:
            if st.session_state.kart_yuzu == "on":
                ...
                if c1.button("🔴 Unuttum", use_container_width=True):
                    ...
                    c.execute(
                        "UPDATE vocabulary SET interval=1, ease_factor=?, next_review=?, correct_streak=0, last_reviewed=? WHERE id=?", 
                        (y_ease, bugun + timedelta(days=1), bugun, k_id)
                    )
                    conn.commit()
                    st.session_state.kart_yuzu = "on"
                    st.rerun()
                ...
                if c2.button("🟡 Zorlandım", use_container_width=True):
                    ...
                    c.execute(
                        "UPDATE vocabulary SET interval=?, ease_factor=?, next_review=?, last_reviewed=? WHERE id=?", 
                        (y_int, y_ease, bugun + timedelta(days=y_int), bugun, k_id)
                    )
                    conn.commit()
                    st.session_state.kart_yuzu = "on"
                    st.rerun()
                ...
                if c3.button("🟢 Kolaydı", use_container_width=True):
                    ...
                    c.execute(
                        "UPDATE vocabulary SET interval=?, ease_factor=?, next_review=?, correct_streak=correct_streak+1, last_reviewed=? WHERE id=?", 
                        (y_int, y_ease, bugun + timedelta(days=y_int), bugun, k_id)
                    )
